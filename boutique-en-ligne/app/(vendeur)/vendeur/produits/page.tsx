...
const emptyVariant = (): Variant => ({ nom: '', couleur: '', stock: '', images: [] })
...
export default function VendeurProduitsPage() {
  ...
  const openEdit = (p: Product) => {
    ...
    setForm({
      ...
      stock: String(p.stock), categoryId: p.category.id, images: p.images, actif: p.actif,
    })
    setPrixTiers(
    ...
    setVariants(
      p.variants.map(v => ({ id: v.id, nom: v.nom, couleur: v.couleur || '', stock: String(v.stock), images: v.images }))
    )
    setActiveTab('infos'); setError(null); setShowForm(true)
  }

  // Upload image produit principal
  ...
  // Upload image pour une variante
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const variantsData = variants
        .filter(v => v.nom.trim())
        .map(v => ({ nom: v.nom, couleur: v.couleur || null, stock: parseInt(v.stock) || 0, images: v.images }))
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openAdd} className="bg-emerald-600 hover:bg-emerald-700 text-white text-sm px-4 py-2 rounded-xl font-medium transition-all">
          + Ajouter
        </button>
      </div>
          ...
          {(['all', 'true', 'false'] as const).map(v => (
            <button key={v} onClick={() => setFilterActif(v)}
              className={`text-xs px-3 py-1.5 rounded-full font-medium transition-all ${
                filterActif === v ? 'bg-emerald-600 text-white' : 'bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700'
              }`}>
              ...
              <div className="p-3">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <span className={`shrink-0 text-xs px-2 py-0.5 rounded-full ${
                    p.actif ? 'bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'
                  }`}>{p.actif ? 'Actif' : 'Désactivé'}</span>
                </div>
                  ...
                  <div className="flex items-center gap-1 mb-2 flex-wrap">
                    {p.variants.slice(0, 6).map(v => (
                      <span key={v.id} title={v.nom}
                        ...
                        {v.couleur && <span className="w-2.5 h-2.5 rounded-full inline-block border border-gray-300" style={{ backgroundColor: v.couleur }} />}
                        {v.nom}
                      </span>
                    ))}
                    {p.variants.length > 6 && <span className="text-[10px] text-gray-400">+{p.variants.length - 6}</span>}
                  </div>
                )}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-xl max-h-[92vh] overflow-y-auto">
                ...
                {(['infos', 'prix', 'variantes'] as const).map(tab => (
                  <button key={tab} onClick={() => setActiveTab(tab)}
                    ...
                    {tab === 'variantes' && variants.length > 0 && (
                      <span className="ml-1 bg-purple-100 dark:bg-purple-900 text-purple-600 dark:text-purple-300 text-[9px] px-1 rounded-full">{variants.length}</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="p-5 space-y-4">
              {error && <div className="p-3 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300 rounded-xl text-sm">{error}</div>}
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      {categories.length === 0 ? (
                        <p className="text-xs text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-950 p-2 rounded-xl">Aucune catégorie approuvée.</p>
                      ) : (
              ...
              {activeTab === 'prix' && (
                <>
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Ce prix est affiché par défaut et barré quand un palier s'applique</p>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button type="button" onClick={() => setPrixTiers(t => [...t, emptyTier()])}
                        className="text-xs bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 px-2 py-1 rounded-lg hover:bg-emerald-200 transition">
                        + Palier
                      </button>
                    </div>
                    ...
                    {prixTiers.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 text-[10px] text-gray-500 dark:text-gray-400 px-1 font-semibold uppercase tracking-wide">
                          <span>Qté min</span><span>Qté max</span><span>Prix/u (DA)</span><span></span>
                        </div>
                          ...
                          <div className="mt-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                            ...
                            <div className="space-y-1">
                              {prixTiers.filter(t => t.minQte && t.prix).map((t, i) => {
                                const reduction = Math.round((1 - parseFloat(t.prix) / parseFloat(form.prix)) * 100)
                                return (
                                  <div key={i} className="flex justify-between text-xs text-gray-600 dark:text-gray-400">
                                    ...
                                    <span className="font-semibold text-gray-800 dark:text-gray-200">
                                      ...
                                      {reduction > 0 && <span className="ml-1 text-green-600 dark:text-green-400">−{reduction}%</span>}
                                    </span>
                                  </div>
                                )
                              })}
              ...
              {activeTab === 'variantes' && (
                <>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-medium text-gray-600 dark:text-gray-300">Couleurs ou parfums</p>
                    ...
                    <div className="space-y-3">
                      {variants.map((v, i) => (
                        <div key={i} className="border border-gray-200 dark:border-gray-700 rounded-xl p-3 space-y-2.5">
                          {/* Ligne 1 : swatch + nom + supprimer */}
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-lg border border-gray-200 dark:border-gray-600 shrink-0 overflow-hidden"
                            ...
                            <input type="text" value={v.nom}
                              ...
                              placeholder="Nom (ex: Rouge, Lavande)" className="flex-1 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-400" />
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                <input type="text" value={v.couleur}
                                  onChange={e => setVariants(vs => vs.map((x, j) => j === i ? { ...x, couleur: e.target.value } : x))}
                                  placeholder="#000000" className="flex-1 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-emerald-400" />
                              </div>
                            </div>
                            <div>
                              ...
                              <input type="number" value={v.stock} min="0"
                                ...
                                placeholder="0" className="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-400" />
                            </div>
                          </div>
                          ...
                          <div>
                            <label className="text-[10px] text-gray-500 dark:text-gray-400 mb-1.5 block">
                              Images — {v.nom || 'variante'} <span className="text-gray-300 dark:text-gray-600">(remplacent les images du produit)</span>
                            </label>
                            <div className="flex flex-wrap gap-2">
                              {v.images.map((img, imgIdx) => (
                                <div key={imgIdx} className="relative group">
                                  <img src={img} alt="" className="w-12 h-12 object-cover rounded-lg" />
                                  <button onClick={() => setVariants(vs => vs.map((x, j) => j === i ? { ...x, images: x.images.filter((_, k) => k !== imgIdx) } : x))}
                                    className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-3.5 h-3.5 text-[9px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">×</button>
                                </div>
                              ))}
                              <button
                                type="button"
                                ...
                                className="w-12 h-12 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-lg flex items-center justify-center text-gray-400 hover:border-emerald-400 hover:text-emerald-400 transition text-lg">
                                {varUploadIdx === i ? (
