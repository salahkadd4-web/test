...
//
// Mode Twilio test :
//   • Si le numéro == TWILIO_TEST_NUMBER  → SMS réel envoyé (Twilio Verify)
//   • Sinon → pas d'envoi, code universel 000000 accepté
...
const TWILIO_TEST_NUM = process.env.TWILIO_TEST_NUMBER // ex: "0XXXXXXXXX"
...
function isTwilioTestNumber(telephone: string): boolean {
  ...
  // Comparer en format local ET international
  return (
    clean === cleanTest ||
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Bloquer si l'utilisateur est déjà VENDEUR ou ADMIN
    ...
    if (etape === 1) {
      ...
      // Vérifier que ce numéro n'est pas déjà pris
      ...
      // Nettoyer les anciens tokens pour cet userId
    ...
    if (etape === 2) {
      ...
      if (otpToken.token === 'TWILIO_VERIFY') {
        ...
        // Mode test : seul 000000 est accepté
        if (code !== UNIVERSAL_CODE) {
          return NextResponse.json({ error: 'Code invalide' }, { status: 400 })
        }
      }

      // ── Mise à jour : rôle CLIENT → VENDEUR + profil ─────────────────────
      await prisma.$transaction(async (tx) => {
        ...
        // 2. Créer le VendeurProfile (EN_ATTENTE par défaut)
